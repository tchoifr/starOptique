...
export async function buildOptimalDynamicTransactions(
  ...
  const [buildTransactions, rbh] = await Promise.all([
    buildOptimalDynamicTransactionsNoSigning(
      connection,
      instructions,
      feePayer,
      priorityConfig,
 ...
 */
export async function buildOptimalDynamicTransactionsNoSigning(
  connection: Connection,
  instructions: InstructionReturn | InstructionReturn[],
  feePayer: AsyncSigner,
  priorityConfig: PriorityConfig,
  ...
  const ixWithComputeBudget = await getIxWithComputeBudget();
